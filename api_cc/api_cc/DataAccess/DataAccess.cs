...
namespace ECommerce.API.DataAccess
{
    ...
    public class DataAccess : IDataAccess
    {
        ...
        public void AddnewProduct(Product product)
        {
            ...
            {
                using (SqlCommand command = new SqlCommand("INSERT INTO Products (Title, Description, CategoryId, OfferId, Price, Quantity, ImageName) VALUES (@Title, @Description, @CategoryId, @OfferId, @Price, @Quantity, @ImageName)", connection))
                {
        ...
        public void UpdateProduct(int ProductId, double Price)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                using (SqlCommand command = new SqlCommand("UPDATE Products SET  Price = @Price WHERE ProductId = @ProductId", connection))
                {
        ...
        public bool InsertAdmin(Admin admin)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = "SELECT COUNT(*) FROM Admins WHERE Email='" + admin.Email + "';";
                command.CommandText = query;
                int count = (int)command.ExecuteScalar();
                if (count > 0)
                {
                ...
                query = "INSERT INTO Admin ( Id, Email, Password) values (@id, @em, @pwd);";

                command.CommandText = query;
                ;
                command.Parameters.Add("@id", System.Data.SqlDbType.NVarChar).Value = admin.Id;
                command.Parameters.Add("@em", System.Data.SqlDbType.NVarChar).Value = admin.Email;
                command.Parameters.Add("@pwd", System.Data.SqlDbType.NVarChar).Value = admin.Password;
        ...
        public string IsAdminPresent(string email, string password)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                var key = configuration["Jwt:AdminKey"];
                var duration = configuration["Jwt:Duration"];
                var symmetrickey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(key));
                var credentials = new SigningCredentials(symmetrickey, SecurityAlgorithms.HmacSha256);
